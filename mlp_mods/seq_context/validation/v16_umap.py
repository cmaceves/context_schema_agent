"""UMAP of v16 embeddings (protein x context), colored by disease and by state. whitegrid."""
...
d = np.load(SEQ / "results/link_v16/embeddings.npz", allow_pickle=True)
...
for ax, lab, title in [(axes[0], disease[sel], "disease"), (axes[1], state[sel], "state")]:
    ...
    ax.set(title=f"v16 embedding (protein x context) — colored by {title}", xlabel="UMAP-1", ylabel="UMAP-2")
    ax.legend(markerscale=3, fontsize=8, loc="best")
fig.tight_layout()
out = SEQ / "images/v16_umap.png"; fig.savefig(out, dpi=130)
