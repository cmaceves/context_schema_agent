"""Pooled protein-DISEASE embedding -> target prediction, boxplot variant of crohn_phase_boxplot.py.

Pipeline (per the requested schematic):
    <protein>_Crohn_contexts  ->  mean pooling  ->  protein-disease embedding  ->  target prediction
i.e. each protein's embedding is averaged over ALL crohn_* contexts it appears in (colon/ileum,
macrophage/fibroblast, every state) to give one vector per protein, then a classifier predicts
drug-targetness. Compared to the same pooling over healthy_* contexts and to raw ESM.

Conditions (color): EMB (Crohn pooled) / EMB (healthy pooled) / ESM. Same drug-target labels + controls,
out-of-fold P (5-fold), L2 LogReg (balanced). See SEQ_CONTEXT_EMBED.md.

Run: .venv_scvi/bin/python mlp_mods/seq_context/validation/crohn_phase_boxplot_pooled.py --run link_v7_4ct
Out: images/crohn_phase_boxplot_pooled.png
"""
...
def pool(d, prefix):
    """mean-pool each protein's embedding over all contexts whose name starts with `prefix`."""
    m = np.array([c.startswith(prefix) for c in d["context"]])
    genes = [PROT[i] for i in d["prot_idx"][m]]
    E = d["emb"][m].astype(np.float64)
    acc = defaultdict(list)
    for g, v in zip(genes, E):
        acc[g].append(v)
    n_ctx = int(m.sum() and len(set(np.array(d["context"])[m])))
    return {g: np.mean(vs, 0) for g, vs in acc.items()}, n_ctx
...
def main():
    ...
    crohn_pool, n_cro = pool(d, "crohn_")
    heal_pool, n_hea = pool(d, "healthy_")
    common = sorted(set(crohn_pool) & set(heal_pool) & set(PROT))
    genes = np.array(common)
    print(f"pooled over {n_cro} crohn_* and {n_hea} healthy_* contexts; proteins in all conditions={len(genes)}",
          flush=True)

    conds = [("EMB (Crohn pooled)", "crohn"), ("EMB (healthy pooled)", "healthy"), ("ESM", "esm")]
    feat = {"crohn": np.stack([crohn_pool[g] for g in genes]),
            "healthy": np.stack([heal_pool[g] for g in genes]),
    ...
    pal = {"EMB (Crohn pooled)": "#1b9e77", "EMB (healthy pooled)": "#a6d8c5", "ESM": "#d95f02"}
    ...
    ax.set(xlabel="", ylabel="P(Crohn target)  [out-of-fold]",
           title=f"Pooled protein-disease embedding -> target prediction\n"
                 f"mean-pool over crohn_* ({n_cro} ctx) vs healthy_* ({n_hea} ctx) vs ESM · "
                 f"{args.run} · {args.classifier.upper()}, 5-fold CV")
