"""UMAP of v15 embeddings (protein x context), colored by disease and by state. whitegrid."""
...
d = np.load(SEQ / "results/link_v15/embeddings.npz", allow_pickle=True)
...
for ax, lab, title in [(axes[0], disease[sel], "disease"), (axes[1], state[sel], "state")]:
    ...
    ax.set(title=f"v15 embedding (protein x context) — colored by {title}", xlabel="UMAP-1", ylabel="UMAP-2")
    ax.legend(markerscale=3, fontsize=8, loc="best")
fig.tight_layout()
out = SEQ / "images/v15_umap.png"; fig.savefig(out, dpi=130)
