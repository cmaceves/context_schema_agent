"""Prep the multi-disease T-cell h5ad (01_expression/new_celltypes/tcell.h5ad) for SCENIC — no scVI, no states.
obs.arm + obs.tissue_slug (set at pull time) → context = <arm>_<tissue>_tcell_all. Gene universe = top-N
variance HVG ∪ present TFs ∪ OT/drug-target genes (same recipe as prep_new_disease.py). Run in .venv_scvi.

Out: seq_context/scenic/inputs/tcell/{genes.txt, tfs.txt, context_cells.tsv, <tag>/counts.npz}
"""
...
INFILE = "mlp_mods/01_expression/new_celltypes/tcell.h5ad"
STATE_MARKERS = SEQ / "scenic/state_markers.tsv"
OUT = SEQ / "scenic/inputs/tcell"
MIN_CELLS = 50
N_HVG = 6000


def tcell_panels():
    sm = pd.read_csv(STATE_MARKERS, sep="\t")
    sm = sm[sm.cell_type == "tcell"]
...
def main():
    ...
    panels = tcell_panels()
    markers = {g for lst in panels.values() for g in lst}
    keep = sorted(hvg | (tr & present) | (tgts & present) | (markers & present))   # force-keep state markers
    ...
    print(f"tcell: genes={len(genes)} (HVG={len(hvg)})  tfs(sources)={len(tfs)}  states={list(panels)}", flush=True)

    # marker-score every T cell against the subset panels -> argmax state
    scores = {}
    ...
    tag = (a.obs.arm.astype(str).values + "_" + a.obs.tissue_slug.astype(str).values + "_tcell_" + state)
    ...
    for t in sorted(set(tag)):
        ...
        print(f"  {'SKIP ' if not keep_ctx else '     '}{t:40s} {len(idx):6d}", flush=True)
