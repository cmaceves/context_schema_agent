"""Prep the multi-disease B-lineage h5ad for SCENIC — no scVI. obs.arm + obs.tissue_slug (set at pull time)
-> context = <arm>_<tissue>_bcell_<state>. Gene universe = top-N variance HVG ∪ present TFs ∪ OT/drug-target
genes ∪ state markers. 5 marker-scored B states from state_markers.tsv (cell_type=bcell). Run in .venv_scvi.

Out: seq_context/scenic/inputs/bcell/{genes.txt, tfs.txt, context_cells.tsv, <tag>/counts.npz}
"""
...
INFILE = "mlp_mods/01_expression/new_celltypes/bcell.h5ad"
STATE_MARKERS = SEQ / "scenic/state_markers.tsv"
OUT = SEQ / "scenic/inputs/bcell"
MIN_CELLS = 50
N_HVG = 6000


def panels():
    sm = pd.read_csv(STATE_MARKERS, sep="\t"); sm = sm[sm.cell_type == "bcell"]
...
def main():
    ...
    print(f"bcell: genes={len(genes)} (HVG={len(hvg)})  tfs(sources)={len(tfs)}  states={list(pan)}", flush=True)
    ...
    tag = (a.obs.arm.astype(str).values + "_" + a.obs.tissue_slug.astype(str).values + "_bcell_" + state)
