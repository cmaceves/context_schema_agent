"""Build the UC (Smillie SCP259) macrophage state networks into the expressed embedding, identically
to the other expressed-backbone disease builds (no special processing):

  for each Leiden state with a DE table (macrophage_uc_smillie_states/states/<state>/pseudobulk_de.tsv):
    - expressed node set = mean CP10k >= 0.5 over that state's cells (precompute_expressed_threshold logic)
    - register a manifest build  macrophage_uc_smillie_<state>  (arms healthy|uc, de_table, no OpenTargets)
    - B.main(..., expressed_backbone=True, rank_weight_all=True)  -> networks/uc_macrophage_<state>/
  plus healthy_uc_macrophage: Smillie NORMAL-arm macrophages, CP10k expressed, OmniPath, NEUTRAL weights
  (same as build_healthy_per_disease.py) -> the UC displacement origin.

Run (after pull_smillie_uc.py + macrophage_uc_smillie_states.py):
  .venv/bin/python mlp_mods/de_ppi/build_uc_smillie.py
"""
...
H5 = str(RS / "macrophage_uc_smillie_paired/pulled_macrophages.h5ad")
STATES = RS / "macrophage_uc_smillie_states"
CP10K_CUTOFF = 0.5
...
def build_neutral(a, tag, mask):                        # expressed node set over `mask` cells, neutral weights
    ...
    print(f"{tag}: {m.n_obs} normal cells, {len(prot)} proteins, {len(e)} edges (neutral)", flush=True)
...
def main():
    ...
    # disease-emergent states (cells exist but per-state DE was skipped: no healthy arm) -> include them
    # as expression networks anyway (expressed node set from the UC cells + neutral weights).
    emergent = [s for s in sc["state"] if s not in states and (a.obs.state == s).sum() > 0]
    print(f"UC states with DE: {states} | disease-emergent (expression-only, neutral): {emergent}", flush=True)
    ...
    for state in states:
        ...
        man["builds"][build] = {
            "cell_type": "macrophage", "disease": "ulcerative colitis",
            "disease_synonyms": ["UC", "inflammatory bowel disease", "colitis"],
            "disease_slug": "uc", "celltype_slug": f"macrophages_{state}",
            "de_table": str(de_tsv),
            "_note": "UC macrophage Leiden STATE sub-build (Smillie SCP259, colon LP). expressed-backbone; "
                     "--no-lit; no OpenTargets. Same processing as the census macrophage builds.",
            "arms": [{"name": "healthy", "h5ad": H5, "disease_filter": None, "target_label": "healthy_macrophage"},
                     {"name": "uc", "h5ad": H5, "disease_filter": "ulcerative colitis", "target_label": "uc_macrophage"}]}
    ...
    for state in emergent:
        build_neutral(a, f"uc_macrophage_{state}", ((a.obs.state == state) & (a.obs.disease.astype(str) == "ulcerative colitis")).values)
    build_neutral(a, "healthy_uc_macrophage", (a.obs.disease.astype(str) == "normal").values)
