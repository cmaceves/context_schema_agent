...
- **In-silico perturbation (scVI, `insilico_perturb/`, inflammatory state, Crohn/UC/ILD).** Two readouts point different ways; both are real:
  ...
  - *Top-of-list recovery* (measured for Crohn colon inflammatory only) — ranking proteins by raw projection (toward-healthy) gives **above-random early enrichment**: at OT>0.3, MRR 0.024 = **6.1× random (perm p=7e-4)**, Hits@10 3/70, Hits@50 8/70; at OT>0.5, MRR **9.7× random (p=0.012)** but 48–88% of it carried by TNF alone. Recall is flat deeper down — **Hits@1000 = 44/70 ≈ the random baseline (33.6)**. So real Crohn NF-κB/TNF-axis targets (NFKB1, TNF, IRF1, PRDM1, RIPK2, TNFAIP3, CD40, PTGS2) do concentrate near the top, **but because they are high-DE / high-degree** (hence the negative residual), not from a target-specific response — and the enrichment does not extend to whole-list ordering. Net: the earlier flat-"null" framing was too strong; the honest statement is *weak, DE/degree-driven top enrichment, no wiring-independent signal.*
...
## 8. `_masked`  — masked-feature-modeling pretraining  *(newest)*
...
- **Not a clean test.** The primary `L_mask` is also ≈0, whereas the standalone `_masked` build (step 8) beat baseline — so this run's failure is confounded by additions relative to step 8: the link anchor competing (loss ~1.58 ≫ L_mask), the **global shared mask set** (same ~1.5k universe nodes masked in every net → co-masked neighbourhoods degrade context), and possibly lr. **Next (to isolate):** reproduce step-8 masked-FM baseline (R²>0) first; then add `L_aux` alone (`--lam-link 0`); then the anchor; consider lr 1e-3 / lower mask_frac / per-net masking. Build artifacts kept in `results/crohn_alzheimer_ild_uc_masked_delta/` for the record.
